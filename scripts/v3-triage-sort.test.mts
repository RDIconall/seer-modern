...
try {
  ...
  assert.equal(triage.rows[4].subject, "Row pending");
  assert.equal(triage.rows[0].category, "ops");
  assert.equal(triage.rows[3].category, "sales");
  assert.equal(triage.rows[4].category, null);
