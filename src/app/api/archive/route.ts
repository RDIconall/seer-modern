...
/** Legacy parity: archive one or more messages. Body: { id } or { ids: string[] } */
export async function POST(request: Request) {
  try {
    ...
    await Promise.all(
      batch.map((id) => run(session.accessToken, id, "archive")),
    );

    return NextResponse.json({ ok: true, archived: batch.length });
  } catch (e) {
